...
- A **valid** key returns **400** on a malformed query (missing `select`, or `depth > 1` without `populate` — see [cms-api-reads](../server/CLAUDE.md)). So: **400 = authenticated but bad query; 403 = not authenticated.**
