...
- **@MCP_USAGE.md** - MCP server best practices for Puppeteer and Cloudflare Docs

## Modular Rules & Docs

Path-scoped guidance lives in **nested `AGENTS.md` files** (each with a `CLAUDE.md` symlink beside it), which are included when the agent reads files in their directory — a nested file's location *is* its scope, so it needs no frontmatter. Reference docs live in `docs/`.
