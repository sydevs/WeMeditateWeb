...
# Re-sync lib/translations.en.json from the CMS English locale.
...
- **Translations are CMS-owned. Never write a user-visible literal.** Every string the site
  ...
  and the fallback when the CMS read fails, never a place to add a string by hand. A new string
  is added to the CMS schema upstream first (`sydevs/SahajCloud`), then reaches this repo through
  `pnpm types:cms` and `pnpm sync:translations`. Two guards under `tests/guards/` enforce this.
...
- **How a URL is spelled lives in [lib/urls.ts](lib/urls.ts), not in a consumer.**
  ...
  against a canonical of `/fr/about`. It skips an href carrying a `?` or a `#`, where a trailing
  slash is the value's and not the path's. Anything else (`mailto:`, an `#anchor`,
  `//another-origin`) is not ours to spell, and passes through untouched.
