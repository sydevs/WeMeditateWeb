 ...
 *
 * This app shipped neither, which meant nothing on it was discoverable except
 * by link-following — and the `/map` routes (#62) exist precisely to be
 * indexable, so they need a sitemap to be worth adding.
 *
 * Registered beside `registerApiRoutes` and, like it, inside the
 * `contextStorage()` middleware, so `getCmsContext()` resolves the API key and
 * KV binding normally. Both routes are declared before Vike's handler so they
 * win over the page catch-all.
 *
 * Every read here degrades to empty rather than failing the response: a partial
 * sitemap is useful, and a 500 on `/robots.txt` is read by some crawlers as
 * "crawl nothing".
 */
...
/** Bounded for the same reason the atlas read is — this runs in a Worker request. */
...
/**
 * Site-content URLs — pages, meditations and lectures — in the default locale.
 *
 * The path shapes mirror `ROUTE_BUILDERS` in `lib/cms-routes.ts`, which owns the
 * inverse direction (`/:slug`, `/meditations/:id`, `/lectures/:id`).
 *
 * Locale variants are deliberately not listed yet: the site serves ten of them,
 * and doing it properly means `xhtml:link` alternates per entry rather than ten
 * times the rows. Worth doing, and worth doing separately.
 */
  ...
  try {
    return await withCache({
      ...
      fetchFn: async () => {
        ...
        return [
          // An unpublished page comes back without a slug; it has no URL to list.
          ...(pages?.docs ?? [])
...
export function registerSitemapRoutes(app: Hono<CmsEnv>): void {
  ...
  app.get('/sitemap.xml', async (c) => {
    ...
    // A preview origin already tells crawlers to stay out; serving it a full
    // sitemap would only invite the indexing robots.txt just refused.
